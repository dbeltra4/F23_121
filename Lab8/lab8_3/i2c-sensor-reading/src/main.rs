...
// ANCHOR: include
use icm42670::{prelude::*,Address, Icm42670, PowerMode as imuPowerMode};
// ANCHOR_END: include
// ANCHOR: shared_bus
...
fn main() -> Result<()> {
    ...
    loop {
        ...
        let acceldata = imu.accel_raw().unwrap();
        //sht.start_measurement(shtPowerMode::NormalMode).unwrap();
        FreeRtos.delay_ms(100u32);
        //let measurement = sht.get_measurement_result().unwrap();
          if acceldata.y > 500 && acceldata.x > 500 {
             println!( "UP LEFT");
          }
          else if acceldata.y  > 500 && acceldata.x < -500 {
             println!( "UP RIGHT");
          }
          else if acceldata.y  < -500 && acceldata.x  > 500 {
             println!( "DOWN LEFT");
          ...
          else if acceldata.x < -500 {
             println!("RIGHT");
          }
          else if acceldata.x > 500 {
             println!("LEFT");
          }
          else if acceldata.y < -500 {
             println!("DOWN");
          }
          else if acceldata.y > 500 {
             println!("UP");
