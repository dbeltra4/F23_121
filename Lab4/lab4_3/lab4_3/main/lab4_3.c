...
int update_mouse(int16_t gyro_cord, int curr, int min_mouse, int max_mouse, int steps){
    if(gyro_cord > -2000 && gyro_cord < 2000){
       return 0;
    }
    if(gyro_cord > 2000 && (curr < max_mouse)){
       return(curr + steps); 
    }
    if(gyro_cord < -2000 && (curr > min_mouse)){
       return(curr - steps);
    }
    return curr;
}
...
void hid_demo_task(void *pvParameters){
     ...
     while(1) {
	       ...
	       // Update movements
	       xM = update_mouse(yGyro, xM, -122, 122, 2);
	       yM = update_mouse(xGyro, yM, -125, 125, -2);

	       printf("xM: %d  yM: %d\n", xM, yM);
	       esp_hidd_send_mouse_value(hid_conn_id, 0, xM, yM);
